...
btnNextLevel.addEventListener("click", data2);

btnLevel.addEventListener('click', pauseTime);
level2.addEventListener("click", data2);
...
function data2 (){
    window.location.assign("../data2/index.html");
}
...
function playPause() {
    if (count == 0) {
        count = 1;
        myMusic.play(); 
        soundBtn.style.backgroundImage = "url(./images/logo/"+1+".png)";
        // alert("music played");
    }else{
        myMusic.pause();
        count = 0;
        soundBtn.style.backgroundImage = "url(./images/logo/"+2+".png)";
        // alert("music paused");
    }
}
